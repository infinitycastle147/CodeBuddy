...
import { api, User, Chat, Diagram, RepoSetup, TaskStatus, Message } from '@/lib/api-endpoints'
...
function useErrorHandler() {
  ...
  return (error: unknown, customMessage?: string) => {
    ...
    if (error instanceof ApiError) {
      ...
      errorMessage = error.data?.error?.message || error.message || `Error ${error.status}: ${error.statusText}`
...
export function useAddMessage() {
  ...
  return useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      queryClient.setQueryData<Chat>(queryKeys.chat(variables.chatId), (oldData) => {
...
export function useUpdateDiagram() {
  ...
  return useMutation({
    mutationFn: ({ diagramId, content }: { diagramId: string; content: string }) =>
