        ...
        <CardTitle className="text-2xl flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-slate-300 text-sm font-normal mt-1">Choose your diagram type and generate instantly</p>
            </div>
          </div>
          <Button
            onClick={handleGenerate}
            disabled={isGenerating || (selectedType === "custom" && !customDescription.trim())}
      ...
      <CardContent className="p-6">
        ...
        {selectedDiagram && (
          <div className="mb-6 p-4 rounded-lg bg-white border border-slate-200">
            <div className="flex items-center gap-3 mb-2">
              <div className="p-2 bg-white rounded-lg shadow-sm">{selectedDiagram.icon}</div>
              <div>
                ...
                <p className="text-slate-600 text-sm">{selectedDiagram.description}</p>
              </div>
            </div>
          </div>
        )}
          ...
          <TabsList className="grid w-full gap-1 h-full grid-cols-3 lg:grid-cols-6 bg-slate-100 dark:bg-slate-800 p-1 rounded-full shadow-inner">
            {categories.map((category) => (
              <TabsTrigger
                key={category}
                value={category}
                className="flex items-center justify-center gap-3 p-2 text-sm font-medium rounded-full transition-all duration-300 ease-out
                 text-slate-600 dark:text-slate-300
                 hover:bg-white hover:shadow-sm dark:hover:bg-slate-700
                 data-[state=active]:bg-white data-[state=active]:text-slate-900 
                 data-[state=active]:shadow-md dark:data-[state=active]:bg-slate-100 dark:data-[state=active]:text-slate-900"
              >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {diagramTypes
                  ...
                  .map((type) => (
                    ...
                    >
                      <Card
                        className={`h-full border-2 transition-all duration-300 ${selectedType === type.id
                      ...
                      >
                        <CardContent className="p-4 flex flex-col items-center text-center gap-3 h-full">
                          <div
                            className={`p-3 rounded-full transition-all duration-300 ${selectedType === type.id
                          ...
                          <div className="flex flex-col gap-2 flex-1">
                            ...
                            <span className="text-xs text-muted-foreground leading-relaxed">{type.description}</span>
                          </div>
                        </CardContent>
                      </Card>
                    </div>
        ...
        {selectedType === "custom" && (
          <div className="mt-6 p-4 bg-gradient-to-r from-violet-50 to-purple-50 rounded-lg border border-violet-200">
            <Label htmlFor="custom-description" className="text-sm font-semibold text-violet-900 mb-2 block">
            ...
            <p className="text-xs text-violet-600 mt-2">
              💡 Tip: Include details about the structure, elements, and purpose of your diagram for best results.
            </p>
          </div>
        )}
      </CardContent>
